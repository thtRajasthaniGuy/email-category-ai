...
export interface EmailSummary {
  summary: string;
  actionItems: string;
  snippet?: string; // For 2-line preview in list
}
...
async function callGeminiAPI(prompt: string, usePro = false): Promise<string> {
...
export async function summarizeEmail(email: string, usePro = false): Promise<EmailSummary> {
  if (!email.trim()) {
    return { summary: 'No content available', actionItems: '', snippet: '' };
  }
...
Email content:
${email.length > 3000 ? email.substring(0, 3000) + '...' : email}

Response format:
Summary: [concise summary]
Action Items: [list actions or "None"]`;

  const content = await callGeminiAPI(prompt, usePro);
  ...
  const summary = summaryMatch?.[1]?.trim() || 'No summary available';
  const actionItems = actionsMatch?.[1]?.trim() || '';
  const snippet = summary.substring(0, 200); // Max ~2 lines for preview
...
export async function summarizeBatchEmails(
  ...
  for (let i = 0; i < emails.length; i++) {
    ...
    try {
      ...
      console.error(`Failed to summarize email ${email.id}:`, error);
      const fallbackSummary: EmailSummary = { summary: 'Error generating summary', actionItems: '', snippet: '' };
      results.push({ id: email.id, summary: fallbackSummary });
      onEmailSummarized?.(email.id, fallbackSummary);
    }
  }
